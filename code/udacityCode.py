...
"""
Modified udacity code:
"""



# Written by Sven
def featureExtractByName(imageName, flip = False, rescale=1):
    image = mpimg.imread(imageName)
    image = np.floor(image * rescale)

    if flip:
        image = np.fliplr(image)

    return featureExtractionTraining(image)

# Settings for find_cars and featureExtractionTraining
orient = 8
pix_per_cell = 8
cell_per_block = 1
spatial_size = (16, 16)
...
X_scaler = joblib.load('../model/scaler.pkl')


# Taken from Udacity lessons and modified
def find_cars(img, ystart, ystop, xstart, xstop, scale):
    draw_img = np.copy(img)
    draw_img_all = np.copy(img)
    listOfBoxes = []
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            if test_prediction == 1:
                ...
                listOfBoxes.append(((xbox_left + xstart, ytop_draw + ystart), (xbox_left + win_draw + xstart, ytop_draw + win_draw + ystart)))
            ...
            if True:
                ...
                ytop_draw = np.int(ytop * scale)
                win_draw = np.int(window * scale)
                cv2.rectangle(draw_img_all, (xbox_left + xstart, ytop_draw + ystart),
                              (xbox_left + win_draw + xstart, ytop_draw + win_draw + ystart), (0, 0, 255), 6)

    return draw_img, draw_img_all, listOfBoxes

# Modified version of the previous function to be used while training classifiers.
def featureExtractionTraining(img):
    ...
    hog_features = hog1#np.hstack((hog1))#, hog2, hog3))
